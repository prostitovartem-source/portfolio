...
function CodeMonitor({ reducedMotion, isMobile, scrollProgress }) {
  ...
  useFrame((state, delta) => {
    ...
    const wobbleY = Math.sin(clockT * 0.35) * 0.22 * idleSpeed + pointer.current.x * 0.45;
    const wobbleX = Math.sin(clockT * 0.27) * 0.1 * idleSpeed - pointer.current.y * 0.22;
    ...
    const targetX = baseX + pointer.current.x * 0.5;
    const targetY = baseY + pointer.current.y * 0.32 - scroll * 1.4;
    ...
    if (rimMatRef.current) {
      const pulse = 0.18 + Math.sin(clockT * (reducedMotion ? 0.3 : 1.4)) * 0.1;
      rimMatRef.current.opacity = pulse * scrollFade;
    }
  ...
  return (
    <Float speed={reducedMotion ? 0 : 1.7} rotationIntensity={0} floatIntensity={reducedMotion ? 0 : 1.6}>
...
function AnimatedParticles({ count, reducedMotion }) {
  ...
  useFrame((state) => {
    ...
    ref.current.rotation.y = state.clock.elapsedTime * 0.06 * speed;
    ref.current.rotation.x = state.clock.elapsedTime * 0.03 * speed;
    const breathe = 1 + Math.sin(state.clock.elapsedTime * 0.6) * (reducedMotion ? 0.005 : 0.07);
    ref.current.scale.setScalar(breathe);
  });

  return (
    <Points ref={ref} positions={particles} stride={3}>
      <PointMaterial transparent color="#7dd3fc" size={0.024} sizeAttenuation depthWrite={false} opacity={0.75} />
    </Points>
  );
}
...
function CameraRig({ reducedMotion, scrollProgress }) {
  ...
  useFrame((state) => {
    ...
    const targetX = pointer.current.x * 0.95;
    ...
    state.camera.position.y += (targetY - state.camera.position.y) * 0.055;
...
function CursorLight({ reducedMotion }) {
  ...
  useFrame((state) => {
    ...
    if (reducedMotion) return;
    const targetX = 3 + pointer.current.x * 3.2;
    const targetY = 3 + pointer.current.y * 3.2;
...
export default function HeroScene({ reducedMotion, isMobile, scrollProgress }) {
  return (
    <Canvas camera={{ position: [0, 0, 6] }} dpr={isMobile ? [1, 1.5] : [1, 2]}>
      ...
      <AnimatedParticles count={isMobile ? 2200 : 8000} reducedMotion={reducedMotion} />
    </Canvas>
  );
}
